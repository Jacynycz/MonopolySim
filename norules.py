...
if len(sys.argv) != 5:
    print ("\nUso:          python norules.py <numplayers> <numrolls> <numsims> <normalize>")
    print (" -- numplayers: Define el número de jugdores para la simulación")
    print (" -- numrolls:  Define el número de tiradas que hace cada jugador")
    print (" -- numsims: Define el número de simulaciónes que ejecuta")
    print (" -- normalize: Usa el valor 0 si cada simulación tiene menos de 100 tiradas")
    print ("               Si cada simulación tiene muchas tiradas utiliza el valor 1 ")
    print (" -- ")
    print ("\n Ejemplo de simulación de 3 jugadores, 10 tiradas cada jugador, 10000 simulaciones sin normalizar\n")
    print ("python norules.py 3 10 10000 0\n")
    quit()
...
zorder = 0)
plt.show()
ax.get_figure().savefig("norules_"+sys.argv[1]+"p_"+sys.argv[2]+"rolls"+sys.argv[3]+"its.png")
